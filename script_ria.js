...
selector.selectAll(".month-option")
    .data(months)
    .enter()
    .append("option")
...
function bubbleChart() {
    function createNodes(key, data) {
        const filteredData = data
            .filter(function (d) { return d[`${key}`] != "NA" });
        const maxSize = d3.max(filteredData, d => d.price);
        const radiusScale = d3.scaleSqrt()
            .domain([0, maxSize])
            .range([5, 55]);
        const myNodes = filteredData.map(d => ({ ...d, radius: radiusScale(+d.price), x: Math.random() * width_bubble, y: Math.random() * height_bubble }))
        return myNodes;
    }
    ...
    let chart = function chart(hemisphere, month, rawData) {
        ...
        const nodes = createNodes(key, rawData);
        d3.selectAll('.critter-info')
            .text('');
        d3.select(".selected")
        ...
        const old_bubbles = elements.select("circle");
        const data_enter = elements.enter()
            .append("g")
            .classed('bubble', true);
        const bubbles = data_enter.append('circle')
            .attr('r', d => d.radius)
        ...
        function bubbleClick(event, d) {
            const bubble = event.target.id
            d3.select('#critter-name')
                .text(`${d.name}`);
            d3.select('#critter-type')
                .text(`Type: ${d.type}`);
            d3.select('#critter-price')
                .text(`Price: $${d.price}`);
            d3.select('#critter-time-availability')
                .text('Time: ' + d[`${key}`]);
            d3.select(".selected")
                .classed("selected", false);
            d3.select('#' + bubble)
            ...
            keys.forEach(key => {
                ...
                if (d[`${key}`] !== undefined) {
                    d3.select(`#critter-${key}`)
                        .text(parsed_key + ': ' + d[`${key}`]);
                }
                else {
                    d3.select(`#critter-${key}`)
                        .text(``)
        ...
        const zoom = d3.zoom()
            ...
            .translateExtent([[-100, -300], [width + 100, height + 300],])
            .scaleExtent([1, 4])
...
async function initialLoad() {
    const fish = await d3.csv("data/fish.csv", function (d) {
        ...
        const s_hemisphere = addMonths(d, "SH");
    ...
    const hemisphereSelector = d3.select("#hemispheres-2")
        .on('change', onChange);
    const monthSelector = d3.select("#months")
        .on('change', onChange);
    function onChange() {
